...
data-generation tool, using the S2-refined homographies from the
Alignment Refiner. This alignment ensures that the signal the
network sees at training time matches the signal it will see at
inference time.
...
from the same track, with no temporal-proximity constraint. The
...
three-pixel mask dilation to handle missed thin strokes. Each call
site instantiates its inpainter independently, so the three can
use different backends or different configurations without
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    \midrule
    Extra learned weights  & $\sim$500~MB & None (reuses SAM) \\
    Speed                  & Slower & Faster \\
    Style generalization   & Training-distribution only & Generic (SAM-based) \\
    Typical failure        & Residual faint strokes & Missed thin strokes \\
    Mitigation             & --- & 3\,px mask dilation \\
    \bottomrule
  \end{tabular}
\end{table}
