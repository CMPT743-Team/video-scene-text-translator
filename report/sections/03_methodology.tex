...
cased exactly so). Between-frame quad geometry is handled by
optical flow rather than by stitching per-frame OCR detections: in
our primary configuration (\texttt{full\_propagation}) the
reference quad (see below) is propagated to every frame of the
track, and the per-frame OCR quads are used only as track-identity
anchors, which avoids inheriting OCR's per-frame geometric noise
into the downstream pipeline. (A \texttt{gaps\_only} mode that
interpolates flow only where OCR missed a frame is also available
but not the default.) We support classical Farneback flow as a CPU
...
substantially smoother trajectories than classical methods and,
in our measurements, running markedly faster than the CPU
Farneback fallback on a GPU host.
...
Figure~\ref{fig:lcm} shows the end-to-end effect of LCM on a
sequence of frames with varying illumination.

\figplaceholder{lcm-result}{0.55\linewidth}%
  {Lighting Correction Module preserves spatially-varying source
   illumination through translation. Each row shows the English
   source frame (left) and its Spanish translation rendered by the
   full pipeline (right). Lighting conditions vary from row to row
   (hand-cast shadows, directional indoor light, diffuse ambient);
   the per-pixel ratio map derived from the inpainted background
   carries the source illumination onto the edited glyph surface
   in each case.}%
  {fig:lcm}
...
Compositing itself uses Poisson blending via
\texttt{cv2.seamlessClone}~\citep{perez2003poisson} by default:
the warped ROI is pasted so its gradient field matches the
destination frame's ambient gradients at the boundary, which
disguises residual lighting mismatches that plain alpha blending
would expose as a visible seam. Feathered alpha blending
$F' = F\,(1-\alpha) + R\,\alpha$, where $F$ is the original frame,
$R$ is the warped propagated ROI, and $\alpha$ is the feathered
alpha mask from S4, is used as a fallback when the source cannot
be pasted entirely within the destination frame centered on the
paste location---an edge case that arises for ROIs near the frame
border.
...
pixel-level noise.
Figure~\ref{fig:bpn:training} visualizes the BPN's behaviour on
evaluation tracks.

\figplaceholder{bpn}{\linewidth}%
  {BPN training behaviour on two evaluation tracks (GREEN, Coffee).
   Each row shows, from left to right, the Reference canonical ROI
   (the sharp input), the Target frame (the actual blurred target),
   the Predicted output produced by rendering the Reference through
   the BPN-predicted blur kernel, and the $3\times$-amplified
   residual between Target and Predicted. Low-amplitude residuals
   indicate that the predicted parameters reproduce the target's
   per-frame blur.}%
  {fig:bpn:training}
...
Figure~\ref{fig:refiner:training} visualizes alignment before and
after the learned residual correction.

\figplaceholder{refiner-network}{0.55\linewidth}%
  {Alignment Refiner training behaviour. Each cell overlays the
   reference and target canonical ROIs into the red and green
   channels of a single image. Residual colour separation indicates
   pixel-level misalignment left by the classical homography fit;
   after applying the Refiner's predicted $\Delta H$ the channels
   fuse toward yellow, showing that per-frame drift has been
   corrected into the shared canonical space.}%
  {fig:refiner:training}
...
Figure~\ref{fig:inpaint} compares the two backends qualitatively on
a representative ROI.

\figplaceholder{inpainting}{\linewidth}%
  {Inpainter backend comparison on the source word ``SHIRTS''. Top
   row: SRNet, the original learned text-removal network, which
   occasionally leaves faint glyph residue. Bottom row: our default
   Hi-SAM stroke segmentation followed by Navier--Stokes inpainting,
   producing a cleaner fill for the compositing step.}%
  {fig:inpaint}
