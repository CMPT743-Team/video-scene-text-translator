...
high-quality multilingual scene text. Orthogonally,
TextMastero~\citep{wang2024textmastero} targets high-quality
editing across diverse languages and styles, and for the
specifically cross-language setting, CLASTE~\citep{yang2023claste}
uses a self-supervised objective to learn cross-script text
editing. Font-transfer methods such as few-shot style transfer
between languages~\citep{li2021fewshotfont} and
Fontify~\citep{xu2026fontify} improve the visual consistency of
glyphs rendered in a script different from the source font. Our
pipeline uses AnyText2 as its editing backend
(Section~\ref{sec:method:s3}). All of these systems operate on
single images and do not address the additional challenges of
temporal coherence, per-frame lighting and blur adaptation, or
propagation of a single edit across many frames of video.
...
reproduction impossible; our pipeline's overall shape follows
STRIVE, as do its two Text Propagation Module components (LCM and
BPN, Sections~\ref{sec:method:s4} and~\ref{sec:method:bpn}), which
are re-implementations of the STRIVE algorithms. The implementation
is independent and differs in several details, the most significant
being a learned alignment refiner that we designed from scratch
(Section~\ref{sec:method:refiner}); smaller differences are noted
in the respective method subsections.
...
Section~\ref{sec:exp:glyph}, and we emphasize that the
