...
in which all detected scene text is replaced by its target-language
translation. The target string is supplied to the text-editing
model as an explicit input, so the intended output is controlled at
the character level at every step of the pipeline; the fidelity of
the final rendered glyphs is bounded by the underlying editor, which
we discuss in Section~\ref{sec:exp:limits}. The system is structured
as a five-stage
...
explicit character-level control over the replacement string that
end-to-end generative models cannot yet match. The two approaches
...
Section~\ref{sec:experiments} presents representative end-to-end
results, a qualitative comparison against a generative baseline,
analysis of what each learned module contributes, and a brief
